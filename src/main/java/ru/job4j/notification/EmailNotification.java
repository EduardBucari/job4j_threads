...
public class EmailNotification {
    ...
    /**
     * Метод emailTo() должен отправлять почту через ExecutorService.
    ...
    /**
     * Метод send() отправляет email
     * @param subject заголовок email
     * @param body текст email
     * @param email email конкретного пользователя.
     */
    public void send(String subject, String body, String email) {
    }

    /**
     * Метод close() закрывает пул и ждёт пока завершаться все задачи.
     */
    public void close() {
        pool.shutdown();
